...
> **Canon errata applied (Canon Resolution Phase 0 v1.0 §6, items 9, 10 & 12).** File slot is computed per Canon §3 (always last). Per item 10, **no rule changes** — T07's path-level spacing wording (20–30% prior-course items from the 2nd course in a path; ≥30% cumulative final course) is hereby canonical and is the source other documents are read against. Tier references use **Loft tier** (Canon §2.2).
...
1. **Intro line** — number of questions, what's covered, retakes allowed. One sentence each.
2. **Questions** — 8–15 items. Every question maps to a stated module objective; nothing untaught appears.
